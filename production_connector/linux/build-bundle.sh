...
if [[ ! -x "${PYTHON_BUILD}" ]]; then
  ...
  echo "Reusing build virtual environment in ${BUILD_VENV_PATH}"
fi
...
"${PYINSTALLER_EXE}" --noconfirm --clean --onedir --contents-directory . --collect-all pymodbus --collect-all snap7 --name rupmes-connector --distpath "${CLI_DIST}" --workpath "${CLI_WORK}" --specpath "${SPEC_ROOT}" "${LINUX_ROOT}/entry_cli.py"
